...
- (void)loadView
{
  if (!self.title)
    self.title = @"Calendar";
  KalView *kalView = [[(KalView *)[[[self class] kalViewClass] alloc] initWithFrame:[[UIScreen mainScreen] applicationFrame] delegate:self logic:logic] autorelease];
  self.view = kalView;
